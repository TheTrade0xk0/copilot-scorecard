...
function findClaude() {
  const candidates = [
    ...
    '/usr/bin/claude',
    `${process.env.HOME || '/root'}/.npm-global/bin/claude`,
...
app.get('/health', (req, res) => {
  ...
  res.json({ status: 'ok', claude: claude || 'not found', pat: process.env.COLOSSEUM_COPILOT_PAT ? 'set' : 'missing', timestamp: new Date().toISOString() });
});
...
app.post('/research', async (req, res) => {
  ...
  if (!colosseum_pat) return res.status(500).json({ error: 'Server misconfigured — missing Colosseum PAT' });

  const claudePath = findClaude();
  if (!claudePath) return res.status(500).json({ error: 'Claude Code not installed on server — please redeploy' });

  console.log(`[research] Starting: ${project_name} | claude: ${claudePath}`);
...
After your research, return ONLY a valid JSON object (no markdown, no explanation, just raw JSON) with this exact structure:
{
  ...
  try {
    ...
    let claudeOutput;
    try { claudeOutput = JSON.parse(stdout); } catch { throw new Error('Failed to parse Claude response'); }
    ...
    if (err.message.includes('401') || err.message.includes('invalid_api_key')) return res.status(401).json({ error: 'Invalid Anthropic API key' });
    if (err.killed || err.message.includes('timeout')) return res.status(408).json({ error: 'Research timed out — try again' });
    if (err.message.includes('rate_limit')) return res.status(429).json({ error: 'Rate limit hit — retry in a moment' });
...
app.listen(PORT, () => {
  ...
  const claude = findClaude();
  console.log(`[server] Claude: ${claude || '✗ NOT FOUND — will fail requests'}`);
});
